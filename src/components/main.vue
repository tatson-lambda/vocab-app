<template>
  <v-container app fluid class="fill-height pa-0">
    <v-row style="height: 100%" no-gutters>
      <v-col cols="8">
        <div class="pa-5">
          <span @click="mouseOver(n)" v-for="(n, i) in formatted" :key="n + i">
            {{ n }}
          </span>
        </div>
      </v-col>
      <v-col cols="4">
        <div>
          <v-card>
            <v-fade-transition>
              <v-list>
                <v-list-item v-for="(n, i) in words" :key="n + i">
                  {{ n }}
                </v-list-item>
              </v-list>
            </v-fade-transition>
...
export default {
  ...
  data: () => ({
    words: [],
    paragraph: ` What is Lorem Ipsum? Lorem Ipsum is simply dummy text of the printing
